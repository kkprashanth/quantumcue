/**
 * Precision-Recall curve chart component using Recharts.
 */
...
import type { PRCurveData } from '@/utils/mockChartData';
...
export function PrecisionRecallChart({ data, jobName }: PrecisionRecallChartProps) {
      ...
      <div className="flex-1 min-h-[350px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart margin={{ top: 20, right: 20, bottom: 50, left: 30 }}>
            <defs>
              <linearGradient id="color-pr-0" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={CHART_COLORS[0]} stopOpacity={0.3} />
                <stop offset="95%" stopColor={CHART_COLORS[0]} stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" vertical={false} />
            <XAxis type="number" dataKey="x" name="Recall" domain={[0, 1]} tick={{ fill: '#94a3b8', fontSize: 12, dy: 8 }} axisLine={{ stroke: 'rgba(255,255,255,0.1)' }} tickLine={false} label={{ value: 'Recall', position: 'insideBottom', offset: -24, fill: '#94a3b8', fontSize: 16, fontWeight: 'bold' }} />
            <YAxis type="number" dataKey="y" name="Precision" domain={[0, 1]} tick={{ fill: '#94a3b8', fontSize: 12 }} tickFormatter={(val) => val === 0 ? '' : val} axisLine={false} tickLine={false} label={{ value: 'Precision', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 16, style: { textAnchor: 'middle' }, offset: 5, fontWeight: 'bold' }} />
            <RechartsTooltip content={<DarkTooltip xLabel="Recall" />} cursor={{ strokeDasharray: '3 3', stroke: 'rgba(255,255,255,0.2)' }} />
            <Area name={jobName || 'PR'} data={seriesData} dataKey="y" fill="url(#color-pr-0)" stroke={CHART_COLORS[0]} strokeWidth={2} type="monotone" isAnimationActive={true} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
