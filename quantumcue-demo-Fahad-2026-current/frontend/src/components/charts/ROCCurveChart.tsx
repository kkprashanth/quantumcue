/**
 * ROC curve chart component using Recharts.
 */
...
import type { ROCCurveData } from '@/utils/mockChartData';
...
export function ROCCurveChart({ data, jobName }: ROCCurveChartProps) {
  ...
  const seriesData = chartDataArrays.length > 0 ? chartDataArrays[0] : [];
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6 pb-2 border-b border-blue-800/30">
        <div className="flex items-center gap-4">
          <h3 className="text-lg font-bold text-white flex items-center gap-2 leading-none">
            ...
            <span className="inline-block align-middle pt-0.5 min-w-fit">ROC Curve</span>
          </h3>
        </div>
      </div>
      <div className="flex-1 min-h-[350px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart margin={{ top: 20, right: 20, bottom: 50, left: 30 }}>
            <defs>
              <linearGradient id="color-roc-0" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={CHART_COLORS[0]} stopOpacity={0.3} />
                <stop offset="95%" stopColor={CHART_COLORS[0]} stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" vertical={false} />
            <XAxis type="number" dataKey="x" name="FPR" domain={[0, 1]} tick={{ fill: '#94a3b8', fontSize: 12, dy: 8 }} axisLine={{ stroke: 'rgba(255,255,255,0.1)' }} tickLine={false} label={{ value: 'FPR', position: 'insideBottom', offset: -24, fill: '#94a3b8', fontSize: 16, fontWeight: 'bold' }} />
            <YAxis type="number" dataKey="y" name="TPR" domain={[0, 1]} tick={{ fill: '#94a3b8', fontSize: 12 }} tickFormatter={(val) => val === 0 ? '' : val} axisLine={false} tickLine={false} label={{ value: 'TPR', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 16, style: { textAnchor: 'middle' }, offset: 5, fontWeight: 'bold' }} />
            <RechartsTooltip content={<DarkTooltip xLabel="FPR" />} cursor={{ strokeDasharray: '3 3', stroke: 'rgba(255,255,255,0.2)' }} />
            <Area name={jobName || 'ROC'} data={seriesData} dataKey="y" fill="url(#color-roc-0)" stroke={CHART_COLORS[0]} strokeWidth={2} type="monotone" isAnimationActive={true} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
